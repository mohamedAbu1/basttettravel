  ...
  return (
    <section className="p-6">
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <button type="button" onClick={saveEvents} disabled={status.saving || status.loading} className="rounded-xl bg-[#c9a34a] px-5 py-3 font-bold text-[#18252b] shadow-lg transition hover:-translate-y-0.5 disabled:opacity-50">
      ...
      {status.loading ? <p className="py-12 text-center opacity-70">Loading seasonal settings...</p> : (
        <div className="grid gap-4 xl:grid-cols-2">
          {events.map((event) => (
            <article key={event.key} className={`rounded-2xl p-5 shadow-xl ${card}`}>
              ...
              <div className="mt-4 flex items-center gap-3">
                ...
                <label className="mt-6 flex items-center gap-2 text-sm"><input type="checkbox" checked={Boolean(event.annual)} onChange={(e) => updateEvent(event.key, { annual: e.target.checked })} /> Repeat yearly</label>
              </div>
            </article>
          ))}
        </div>
